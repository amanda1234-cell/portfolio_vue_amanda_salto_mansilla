...
import { ArrowLeft, X, Maximize2, ChevronLeft, ChevronRight, HeartPulse } from "lucide-vue-next";
...
const proyecto = {
  ...
  year: "2026",
  description:
    "Desarrollo integral de la campaña visual para el I Congreso Iberoamericano de Creación y Fabricación Digital.",
  secciones: [
    {
      titulo: "Póster Principal y Gráfica",
      icono: HeartPulse,
      items: [{ src: "/proyectos/crefad/poster-principal.jpg", alt: "Póster Campaña C-FAB 2026" }],
    },
  ],
};
</script>
...
<template>
  <div class="page-wrapper Crefad-editorial">
    <section class="detalle-kaoka">
      ...
      <header class="header-editorial Crefad-header">
        <div class="header-content">
          <span class="categoria-tag">{{ proyecto.category }}</span>
          <h1 class="titulo-kaoka Crefad-titulo">{{ proyecto.title }}</h1>
          <p class="descripcion-kaoka">{{ proyecto.description }}</p>
        </div>
        <div class="header-meta">
          <p><strong>Año:</strong> {{ proyecto.year }}</p>
          <p><strong>Ubicación:</strong> Universitat Politècnica de Valencia</p>
        </div>
      </header>
      ...
      <div class="galerias-container">
        <div v-for="(seccion, index) in proyecto.secciones" :key="index" class="bloque-seccion">
          ...
          <div class="grid-ajustado Crefad-grid" :class="`items-${seccion.items.length}`">
            <div
            ...
            >
              <div class="contenedor-estandar Crefad-contenedor">
